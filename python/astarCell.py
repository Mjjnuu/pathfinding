...
while cell_heap.get_size() > 0:
	...
	current_dist = current_cell.get_dist() - current_cell.get_heuristics(target)
	...
	if i-1 >= 0 and grid[i-1,j] != 3 and grid[i-1,j] != 1: 
		...
		hScore = new_cell.get_heuristics(target)
		new_cell.set_dist(hScore + current_dist + 1)
		index = new_cell.search_array(heap_vector)
		if index == None:
			#The heap doesn't contain this neighbor, so add it
			cell_heap.add(new_cell)
			grid[i-1,j] = 2
		elif hScore + current_dist + 1 < heap_vector[index].get_dist(): 
...
plt.imshow(grid, cmap=colormap, norm=norm)
print("A*-algorithm run time:", end=" ")
